...
function pickGeo(mode, manualCoord, anchor) {
  ...
  if (mode === "wfh") return { lat: -6.343049224430671, lng: 106.85907517136334 };
  return randItem(WFO_PRESETS.kalibata);
}
...
function renderGeoCoords(mode, anchor, manual) {
  ...
  if (mode === "manual" && parseGeoCoord(manual)) {
    ...
    el.textContent = `Manual · ${c.lat.toFixed(6)}, ${c.lng.toFixed(6)}`;
    return;
  }
  const a = ANCHORS[anchor] || ANCHORS.kalibata;
  el.textContent = `${mode.toUpperCase()} · ${anchor[0].toUpperCase()+anchor.slice(1)} · ${a.lat.toFixed(6)}, ${a.lng.toFixed(6)}`;
}
